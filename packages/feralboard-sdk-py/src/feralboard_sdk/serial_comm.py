"""Threaded serial communicator for the FeralBoard TX/RX loop."""
...
class SerialCommunicator:
    ...
    def set_output_bit(self, tx_byte: int, bit_idx: int, value: bool):
        attr_map = {
            0: "tx_byte0",
            1: "tx_byte1",
            2: "tx_byte2",
        ...
        if attr is None:
            raise ValueError(f"Unsupported TX byte for output: {tx_byte}")

        with self.lock:
            current = getattr(self, attr)
            if value:
                setattr(self, attr, current | (1 << bit_idx))
            else:
                setattr(self, attr, current & ~(1 << bit_idx))

    def set_output(self, channel_name: str, value: bool):
        _, tx_byte, bit_idx, _ = get_output(channel_name)
        self.set_output_bit(tx_byte, bit_idx, value)
